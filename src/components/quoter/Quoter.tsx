...
async function generateSequentialFolio(
  ...
  let seq = 1;
  try {
    const { data: count, error } = await supabase.rpc("get_quote_count_for_year", {
...
export function Quoter() {
  ...
  const [level, setLevel] = useState<Level | undefined>(
    () => (ls("kt-quote-level") as Level) || undefined,
  );
  const [service, setService] = useState<ServiceType | undefined>(
    () => (ls("kt-quote-service") as ServiceType) || undefined,
  );
  const [city, setCity] = useState<City | undefined>(
    () => (ls("kt-quote-city") as City) || "ensenada",
  );
  ...
  const [stolaColor, setStolaColor] = useState<string>(
    () => ls("kt-quote-stola-color") || "dorada",
  );
  const [productCategory, setProductCategory] = useState<"estolas" | "birretes" | "borlas">(
    () => (ls("kt-quote-product-category") as "estolas" | "birretes" | "borlas") || "estolas",
  );
  ...
  const total = useMemo(
    () => unitPrice(pkg, level, service) * quantity,
    [pkg, level, service, quantity, pricesLoaded],
  );
      ...
      if (step === 5 && !isSaved && !isSaving && school && contact && phone && turnstileToken) {
        ...
        try {
          ...
          const res = await fetch(
            "https://qqfqwvxmhiacoyqwobjd.supabase.co/functions/v1/submit-quote",
            {
              method: "POST",
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col">
            ...
            <div className="absolute left-4 sm:left-6">
              ...
              >
                ...
                <span className="hidden sm:inline">
                  {step === 5 ? "Nueva cotización" : "Regresar"}
                </span>
              </Button>
            </div>
          )}
      ...
      <main className="w-full pt-6 pb-16 flex-1">
        ...
        {((level && step > 1) || (service && step > 2) || ((city || pkg) && step > 3)) &&
          step < 5 && (
              ...
              <div className="flex flex-wrap gap-2">
                {level && step > 1 && (
                  <button
                    type="button"
                    onClick={() => setStep(1)}
                    className="px-3 py-1 bg-white border border-border rounded-full text-[10px] uppercase tracking-wider font-bold text-muted-foreground/70 flex items-center gap-2 hover:bg-muted/50 transition-colors cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                  >
                ...
                {service !== "venta" &&
                  ((pkg?.kind === "A" && level === "preescolar") || level !== "preescolar") &&
                  togaColor &&
                  step > 3 && (
                    <button
                      type="button"
                ...
                {total > 0 && step > 3 && (
                  <button
                    type="button"
                    onClick={() => setStep(3)}
                    className="px-3 py-1 bg-navy border border-navy rounded-full text-[10px] uppercase tracking-wider font-bold text-navy-foreground flex items-center gap-2 hover:opacity-90 transition-opacity cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                  >
