...
export function StepSummary({
  ...
  const rows = [
    ...
    {
      label: "Sede / Ciudad",
      value: city === "tijuana" ? "Tijuana" : city === "ensenada" ? "Ensenada" : cityLabel(city),
      step: 3,
    },
  ...
  if (level) {
    const selectedToga =
      level !== "preescolar" || pkg?.kind === "A" ? colorLabel(togaColor) : "Negro";
    const stolaVal = stolaLabel(stolaColor);

    const packageIndex = rows.findIndex((r) => r.label === "Paquete");
    if (packageIndex !== -1) {
      if (service === "venta") {
        rows.splice(packageIndex + 1, 0, { label: "Estola", value: stolaVal, step: 3 });
      } else {
        rows.splice(
          packageIndex + 1,
          0,
          {
            label: "Color Toga",
            value: selectedToga,
            step: pkg?.kind === "A" || level !== "preescolar" ? 3 : null,
          },
      ...
      {!quoteNumber ? (
        <div className="flex flex-col items-center justify-center py-20 animate-pulse">
          ...
          <p className="text-muted-foreground font-medium">
            Asignando folio oficial y guardando datos...
          </p>
        </div>
      ) : (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 grid grid-cols-1 lg:grid-cols-2 gap-x-16 gap-y-12 items-start">
          ...
          <div className="border-t border-hairline order-1">
            {rows.map((r) => (
              <div
                key={r.label}
                className="flex items-baseline justify-between gap-6 py-4 border-b border-hairline group"
              >
          ...
          <div className="order-3 lg:order-2 pt-8 lg:pt-0 lg:pl-12 lg:border-l lg:border-hairline border-t lg:border-t-0 border-hairline flex flex-col items-center text-center h-full justify-center">
            ...
            <p className="max-w-md text-sm text-muted-foreground leading-relaxed mb-10">
              Nos encantará acompañarlos en este momento tan especial y crear recuerdos que se
              queden para siempre.
            </p>
            ...
            <div className="mt-12 space-y-1">
              ...
              <p className="text-[10px] text-muted-foreground/40 font-medium">2026</p>
            </div>
          </div>
